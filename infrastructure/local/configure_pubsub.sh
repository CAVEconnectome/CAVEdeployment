...
gcloud pubsub subscriptions create ${SKELETON_CACHE_LOW_PRIORITY_RETRIEVE_QUEUE} --topic=${SKELETON_CACHE_LOW_PRIORITY_EXCHANGE} --topic-project=${PROJECT_NAME} --ack-deadline=180 --expiration-period="never" --max-retry-delay=10

gcloud pubsub topics create ${SKELETON_CACHE_HIGH_PRIORITY_EXCHANGE}
gcloud pubsub subscriptions create ${SKELETON_CACHE_HIGH_PRIORITY_RETRIEVE_QUEUE} --topic=${SKELETON_CACHE_HIGH_PRIORITY_EXCHANGE} --topic-project=${PROJECT_NAME} --ack-deadline=180 --expiration-period="never" --max-retry-delay=10
